...
model_efficientnet = models.efficientnet_b1(weights="IMAGENET1K_V1")

for param in model_efficientnet.parameters():
    param.requires_grad = False


num_ftrs = model_efficientnet.classifier[1].in_features  # EfficientNet'in son katmanı
model_efficientnet.classifier[1] = nn.Linear(num_ftrs, len(siniflar))  # Kather Texture 5K veri seti için 8 sınıf

model_efficientnet = model_efficientnet.to(device)

criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(model_efficientnet.classifier[1].parameters(), lr=0.001, momentum=0.9)
