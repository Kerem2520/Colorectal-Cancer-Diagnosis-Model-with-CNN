...
transform = transforms.Compose([
    transforms.Resize((227, 227)),  # AlexNet giriş boyutu
    transforms.ToTensor(),
...
model_alexnet = models.alexnet(weights="IMAGENET1K_V1")

for param in model_alexnet.parameters():
    param.requires_grad = False

# Son Katmanı Değiştirmek
num_ftrs = model_alexnet.classifier[6].in_features  # AlexNet'un son katmanı
model_alexnet.classifier[6] = nn.Linear(num_ftrs, len(siniflar))  # Kather Texture 5K veri seti için 8 sınıf

model_alexnet = model_alexnet.to(device)

criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(model_alexnet.classifier[6].parameters(), lr=0.001, momentum=0.9)
